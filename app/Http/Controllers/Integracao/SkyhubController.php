...
class SkyhubController extends Controller implements Integracao
{
    ...
    public function importPedido($order) {
        try {
            ...
            Log::critical(logMessage($e, 'Pedido ' . ($order['code'] ? $order['code'] : '') . ' não importado'), $order);
            reportError('Pedido ' . ($order['code'] ? $order['code'] : '') . ' não importado: ' . $e->getMessage() . ' - ' . $e->getLine());
            return false;
        }
    }
