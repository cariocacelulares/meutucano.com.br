...
class OrderInvoiceDevolutionController extends Controller
{
    ...
    private function getImeis()
    {
        ...
        foreach ($positions as $key => $pos) {
            ...
            $lineProduct = substr($infCpl, $pos, (stripos($infCpl, $posFind, $pos) - $pos));
            $skuProduct  = (int)substr($lineProduct, stripos($lineProduct, '.: ') + 3, 5);
            $imei        = trim(substr($lineProduct, stripos($lineProduct, 'S/N') + 5));
            ...
            if ($imei) {
                ...
                foreach (explode(',', $imei) as $imei) {
                    ...
                    if ($productImei->productStock->product_sku != $skuProduct) {
                        throw new \Exception("O imei {$imei} está registrado no produto {$productImei->productStock->product_sku} e não no {$skuProduct}.", 7);
                    }
